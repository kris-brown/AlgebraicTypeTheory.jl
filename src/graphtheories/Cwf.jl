...
cmpdecl = OpDecl(:⋅, "binary", Hom_ac, [f,g], "Composition, only defined for pairs of morphisms that match head-to-tail, is an associative operation which picks out a third.")
...
Q = mkApp(:q, [AyΓα])
peq = Rule("Universal property of 𝐩", γ, mkApp(:⋅, [mkApp(:snoc, [γ,N]),P]))
qeq = Rule("Universal property of 𝐪", N, mkApp(:Elact, [Q,mkApp(:snoc, [γ,N])]))
pqeq = Rule("𝐩𝐪 property", mkApp(:id, [ΓA]), mkApp(:snoc, [P,Q]))
cct1 = mkApp(:⋅, [δ,mkApp(:snoc, [γ,N])])
cct2 = mkApp(:snoc, [mkApp(:⋅, [δ,γ]), mkApp(:Elact, [N,δ])])
...
pild22= mkApp(:lift,[p,B])
pild2 = mkApp(:Pi, [Lift,pild22])
piliftdecl = Rule("Π lifting", mkApp(:lift,[p,ΠAB]), pild2)

pisub22 = mkApp(:Tyact,[B,mkApp(:snoc,[mkApp(:⋅,[P,γ]),Q])])
pisub2 = mkApp(:Pi,[AyΓαγ, pisub22])
pisubdecl = Rule("Pi substitution", mkApp(:Tyact,[ΠAB,γ]), pisub2)


cwf = extend(cwf, [], [pidecl, lamdecl], [pisubdecl], "cwf7")
print(render(cwf))

# ERRROR WHEN WE DO infer(pild2)
